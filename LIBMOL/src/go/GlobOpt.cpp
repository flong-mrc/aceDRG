...
namespace GO
{
        ...
        if(tSize <= 1)
        {
                    ...
                    iAt !=allAtoms.end(); iAt++)
            {
                for (std::vector<LIBMOL::REAL>::iterator iCo=iAt->coords.begin();
                         iCo !=iAt->coords.end(); iCo++)
                {
                    LIBMOL::REAL tShift=0.0;
                    tShift = r_delta*LIBMOL::GetRand();
                    *iCo += tShift;
                                                  ...
                                                  LIBMOL::REAL** pos_m)
    {
        ...
        if(workSpace == 1)      // test Cartesian space first
        {
            ...
            {
                //std::cout << "numAtom " << numAtoms << std::endl;
                //std::cout << "dim " << dim << std::endl;
                int tOptSize = (int)allOptimSets.size();
                // std::cout << "n found already " << tOptSize << std::endl;
                if (tOptSize > 0)
                {
