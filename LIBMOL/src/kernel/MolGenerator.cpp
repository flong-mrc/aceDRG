...
namespace LIBMOL
{
                                      ...
                                      PeriodicTable & tPTab)
    {
        ...
        */
        ...
        {   
            ...
            if (checkUniqueAtom(tAtom.fracCoords))
            {
            tAtom.sId = "555";
                ...
                for (int j=-myNBDepth; j <myNBDepth+1; j++)
                {
                    for (int k=-myNBDepth; k <myNBDepth+1; k++)
                    {
                        ...
                        if(!(i==0 && j==0 && k==0))
                        {
                            ...
                            if ((aAtom.fracCoords[0] > -0.8 && aAtom.fracCoords[0] <=1.8)
                                && (aAtom.fracCoords[1] > -0.8 && aAtom.fracCoords[1] <=1.8)
                                && (aAtom.fracCoords[2] > -0.8 && aAtom.fracCoords[2] <=1.8)
                                && !colidAtom(aAtom, refAtoms))
                            {
                                      ...
                                      std::vector<CrystInfo>::iterator tCryst)
    {
        ...
        REAL covalent_sensitivity;
        REAL covalent_sensitivity1 =0.10;
        REAL covalent_sensitivity2 =0.15;
                                     ...
                                     std::vector<CrystInfo>::iterator  tCryst)
    {
                  ...
                  iMol !=moleculesInCell.end(); iMol++)
        {
            ...
            //{
                ...
                std::cout << "The number of atoms in this molecule is " 
                          << aMol.atoms.size() << std::endl;
                       //   << "They are : " << std::endl;
               /*
               for (std::vector<AtomDict>::iterator iAt=aMol.atoms.begin();
                       iAt !=aMol.atoms.end(); iAt++)
               ...
               */    
               ...
               if ( aPass && aMol.bonds.size() >0 )
               {
                   ...
                   std::cout << "Molecule " << iMol->first << " is included "
    ...
    void MolGenerator::outTableBAndA(FileName tBAndAFName)
    {
        ...
        if (aBAndAF.is_open())
        {
            if (bonds.size() >0)
            {
                aBAndAF << "Atom1_COD_type        Atom2_COD_type          Atom1_chem_type     Atom2_chem_type     Bond_length"
                        << std::endl;
