...
namespace LIBMOL
{
    ...
    bool SmileTool::isValidAtomName(ID tId)
    {
        bool aT = false;
        
        std::map<ID,std::vector<int> >::iterator iFd= OrganSet.find(tId);
        if (iFd !=OrganSet.end())
        {
            aT = true;
        }
    ...
    void SmileTool::SmileToMol(ID & tSS, std::vector<AtomDict>& tMol)
    {
