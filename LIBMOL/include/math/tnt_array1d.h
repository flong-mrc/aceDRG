...
class Array1D 
{
  ...
  public:
	...
	inline   T         operator*(const Array1D &A);
	...
	Array1D vectmult(const Array1D &A);
               ~Array1D();
...
/**
        scale product with another Array1D object	
*/

template <class T>
T  Array1D<T>::operator*(const Array1D<T> &A)
{
  int i;
...
/**
        vector product with another Array1D object
        size 3	
*/
  
template <class T>
Array1D<T> Array1D<T>::vectmult(const Array1D<T> &A)
{
  ...
  if(dim() !=3 || A.dim() !=3)
    {
      // temporarily
      std::cerr << "the size of one dimentional vector is not 3 "
           << std::endl;
      exit(1);
    }
  ...
  v_tm[0] = v_[1]*A[2]-v_[2]*A[1];
  v_tm[1] = v_[2]*A[0]-v_[0]*A[2];
  v_tm[2] = v_[0]*A[1]-v_[1]*A[0];
