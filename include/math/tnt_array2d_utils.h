...
/**
	Matrix Multiply by a column vector:  compute C = A*B, where C[i]
	is the dot-product of row i of A and column vector B.
	

	@param A an (m x n) array
	@param B an (n x 1) array
	@return the (m x 1) array A*B, or a null array (0x0)
		if the matrices are non-conformant (i.e. the number 
		of columns of A are different than the number of rows of B.)
...
*/
template <class T>
Array1D<T> matmultvec(const Array2D<T> &A, const Array1D<T> &B)
{
	if (A.dim2() != B.dim1())
		return Array1D<T>();
