...
class Acedrg(CExeCode ):
    ...
    def outEnergyGeoMap(self, tIdxMol):
        ...
        try: 
            ...
            aListF.write("REFMAC-Energy\tOriginal-Conformer-Id\tPDB-File-Name\n")
            for aPair in self.refmacMinFValueList:
...
class AcedrgRDKit():
    ...
    def setInitConformersOneMol(self, tMol):
        ...
        for aId in confIds:
            #print "initial Id ", aId 
            aFailure = self.optOneConformer(tMol, self.numRDKitOptmSteps, self.nMaxIters, aId)
            if not aFailure:
                #print "opted id ", aId
                aForceField = AllChem.UFFGetMoleculeForceField(tMol, confId=aId)
                aEng        = aForceField.CalcEnergy()
                if not self.conformerEngMap.has_key(aEng):
                    self.conformerEngMap[aEng] = []
                self.conformerEngMap[aEng].append(aId)
        ...
        if not self.useExistCoords: 
            ...
            self.selecConformerIds =[0,1]
        print "Number of conformers selected for refinement is ",  len(self.selecConformerIds)

    def setExistCoords(self, tMol, tReadInAtoms, tReadInBonds):
    ...
    def MmCifToMolFile(self, tInFileName, tOutMolName, tMode=0):
        ...
        try:
            ...
            for aAtom in self.atoms:
                ...
                if aAtom.has_key("_chem_comp_atom.pdbx_model_Cartn_x_ideal"):
                ...
                if aAtom.has_key("_chem_comp_atom.pdbx_model_Cartn_y_ideal"):
                ...
                if aAtom.has_key("_chem_comp_atom.pdbx_model_Cartn_z_ideal"):
